...
It talks to model providers, stores Pioneer data, runs tools, manages skills and MCP servers, owns tasks, and streams results back to connected clients. You can run a gateway on the same computer as the desktop app, or on another machine.
...
Provider credentials are configured on the gateway. If you connect to a second gateway, it needs its own provider setup.
