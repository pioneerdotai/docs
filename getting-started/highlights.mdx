...
In agent mode, Pioneer can recall durable facts, preferences, project decisions, and recurring instructions across turns when memory is enabled. Memory combines prompt policy, model-visible memory tools, proactive post-turn extraction, service-owned dedupe, and memvid-backed search capsules.
...
Files are workspace-scoped gateway artifacts. User uploads are copied to the gateway before a turn, and agent-created result files must be registered with the artifact tools before the final response. Historical messages and recalled thread snippets can carry lightweight artifact refs, and the model reads file content on demand through `artifact_read` instead of receiving every old attachment automatically.
