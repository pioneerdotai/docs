...
Remember that the desktop app is only the client. If you connect to a gateway on a server, tools run on that server. Turn permission modes and sandbox/resource policy control what Pioneer allows, asks about, audits, and blocks within that gateway environment.
