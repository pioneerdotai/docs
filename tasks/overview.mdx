...
<Warning>
  Task tools run on the gateway host and follow the task or turn permission profile. Test manual runs in **Supervised** mode before adding schedules or unattended write-capable workflows.
</Warning>
...
This affects file paths, credentials, network access, installed commands, and side effects. Before scheduling a task, make sure you are on the gateway where you actually want the work to happen and that the task's permission profile matches the level of supervision you expect.
...
- [Permission System](/architecture/permissions) explains task permission caps and approval prompts.
