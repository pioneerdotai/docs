...
Pioneer reads the JSON, validates each server under `mcpServers`, stores secrets through the gateway secret store, creates the server entries, and starts or connects to enabled servers.
...
Pioneer stores secret values from `env`, `headers`, tokens, and authorization fields through the gateway secret store and redacts them from diagnostics and source references.
...
Review the discovered tools before enabling them. Keep new MCP tools not implicit until you understand what they can read, write, or call.
