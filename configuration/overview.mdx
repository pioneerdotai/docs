...
Most users can start with the desktop or mobile app and provider settings. Use config files when you need to change gateway behavior, bind addresses, tool limits, database settings, CLI runtime defaults, remote-access relay settings, or development paths.
...
<Warning>
  Tool limits and permission modes reduce risk in different ways. Limits stop runaway loops; permission modes ask before sensitive actions. Approved tools still run on the gateway host.
</Warning>
...
- [Gateway Architecture](/architecture/gateway) explains which systems read gateway settings at runtime.
- [Persistence](/architecture/persistence) explains `gateway.db`, runtime home, and migration behavior.
- [Secret Storage](/architecture/secrets) explains why secret values are stored outside ordinary config.
- [Protocol Layer](/architecture/protocol) explains how clients use `settings/get` and `settings/update`.
