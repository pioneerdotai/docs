...
<Warning>
  Pioneer is moving quickly. Test new workflows on low-risk projects first, especially before exposing a gateway outside your machine or letting agents work unattended.
</Warning>

<Note>
  Agent turns have permission modes. Use **Supervised** when you want Pioneer to ask before commands and file changes, or **Auto-accept edits** when file edits are okay but other actions should still ask. Tools still run on the gateway host, so use a separate gateway when you need a separate machine or account boundary.
</Note>
