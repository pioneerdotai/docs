...
Provider methods are deliberately small. The gateway owns provider configuration, stores credentials in its keystore, discovers models, and normalizes model metadata for clients. Provider configuration is workspace-scoped: every provider method takes `workspace_id`, and API keys are stored under workspace-specific secret ids. A desktop app, CLI, mobile app, or custom client should not talk directly to every model provider just to build a model picker.
