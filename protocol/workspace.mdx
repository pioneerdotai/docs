...
This is a replay surface for clients, not the semantic recall surface used by the agent. Clients should not use `thread/history` to push whole old transcripts back into a model request. Agent-side recall of older discussion is handled by thread episodic context: the gateway indexes visible conversation fragments, searches them when needed, and injects bounded snippets into prompt context.
