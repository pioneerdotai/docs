...
Tasks are the durable automation layer in the gateway. A task can run immediately, wait for a schedule, repeat on an interval or cron expression, depend on another task, or be triggered manually or externally. When the executor is `agent`, the task can create a subagent thread with its own prompt, model, context policy, tool policy, result contract, and review policy.
