...
When a task creates a child thread, task events include child thread lineage. Clients can then show the subagent's work in the parent task tree and can load the child conversation through [Threads API](/protocol/threads) and [Turns API](/protocol/turns).
