...
Clients should refresh thread artifact lists on `thread/artifacts/changed`, as well as on `artifact/created`. An artifact may gain new bindings or metadata without getting a new artifact id.
