...
Thread notifications are not a replacement for read methods. Treat them as invalidation and streaming hints, then call `thread/get`, `thread/tree`, `thread/history`, `thread/timeline/page`, or `turn/work/page` as needed.
