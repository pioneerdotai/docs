...
The important detail is where the provider is saved. Provider configuration belongs to the current workspace on the gateway, not to the desktop app. Secret API keys and tokens are stored in that gateway's `keystore.db` under workspace-scoped secret ids. If you configure a provider in `Default Workspace` and then switch to `Client A`, `Client A` needs its own provider configuration and credentials. If you switch to another gateway, configure providers there too.
