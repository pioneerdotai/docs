...
The important detail is where the provider is saved. Provider configuration belongs to the gateway, not to the desktop app. If you configure a provider on `Local` and then switch to `Work Gateway`, the work gateway needs its own provider configuration.
