...
Artifacts are files owned by a Pioneer workspace. You can upload them, or an agent, tool call, or task can create them. The gateway stores each file and links it to the thread, turn, and message where it was used or created.

This distinction matters when you connect to a remote gateway. A path in a tool call may belong to the remote machine, while an artifact can be downloaded to your desktop and opened there.
...
When an agent creates a file for you, it must register that file with the artifact store before the final response. Registered files link back to the turn and appear in the artifacts panel.
...
Pioneer does not scan arbitrary gateway folders for files to publish as artifacts. If an agent says it created a file but the file is missing from the artifacts panel, ask it to register the file. A file written to a path you specified remains a normal gateway file until registration.
...
This preserves continuity without adding every file to the prompt. If a follow-up does not need the file, the model can answer from the conversation history. If it does, it can request that file instead of receiving every artifact from the thread.
...
- [Artifacts Architecture](/architecture/artifacts) explains artifact storage, bindings, uploads, downloads, projections, and `artifact_read`.
- [Thread Episodic Context](/architecture/thread-episodic-context) explains why recalled snippets can carry artifact references.
- [Tools Architecture](/architecture/tools) explains how model-facing tools are exposed during agent turns.
