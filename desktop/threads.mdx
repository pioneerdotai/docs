...
<Note>
  Tool calls execute on the gateway host. In a remote gateway thread, commands and file paths belong to the remote machine. Use the composer permission mode to decide whether Pioneer should ask before commands, edits, network actions, and other sensitive tool calls.
</Note>
