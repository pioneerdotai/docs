...
`AGENTS.md` is Pioneer's thread-tree instruction layer. Users can place persistent prompt instructions at the workspace root or in a thread-tree folder. New turns resolve the nearest active file automatically.

The gateway owns the implementation. The desktop renders and edits the files; the gateway stores them, resolves inheritance, emits notifications, and injects the effective file through the hook runtime.
