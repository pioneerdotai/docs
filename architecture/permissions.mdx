...
Pioneer applies security per turn. A client selects a permission profile, the gateway resolves it into a `TurnExecutionSecuritySnapshot`, and the tools runtime enforces that snapshot before side effects run.
