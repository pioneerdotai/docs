...
flowchart TD
  ...
  Desktop["desktop-settings.toml\ncrates/desktop/src/settings.rs"]
...
The desktop settings are a product control surface for the same memory switches. They should not create a second memory policy language. If a setting is exposed to users, it must map to a real gateway/hook behavior or stay hidden.
...
sequenceDiagram
  ...
  Model-->>Agent: tools.visibleTools + memory.activeRecall
  Agent->>Hooks: turn.post_preflight_prompt_context
  Hooks->>Memory: active recall from preflight plan or memory fallback
...
6. If the expected context lives only in raw thread history, it will not be recalled until the episodic/thread transcript system exists.
