...
The agent loop lives in `crates/agent`. It is the execution engine for a thread turn after the gateway has accepted `turn/start`, persisted the initial state, selected the provider/model, and materialized the turn permission profile.
...
sequenceDiagram
  ...
  Gateway->>Agent: StartTurn(history, input, capabilities, model, provider, permission_profile)
  ...
  loop provider/tool rounds
    ...
    Agent->>Tools: evaluate permissions and execute tool calls
...
The permission profile is also carried through this path. It is not a prompt-only hint: the tools runtime receives it in `PermissionEvaluationContext` and enforces `allow`, `ask`, or `deny` before side effects. The prompt section exists so the model can plan around user approval boundaries, not so the model becomes the enforcement mechanism.
...
- [Permission System](/architecture/permissions) explains turn permission profiles, approval prompts, audit events, and task permission caps.
