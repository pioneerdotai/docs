...
The agent loop lives in `crates/agent`. It is the execution engine for a thread turn after the gateway has accepted `turn/start`, persisted the initial state, selected the provider/model, materialized the turn permission profile, and resolved the turn execution security snapshot.
...
The agent loop coordinates model calls and side effects. It decides which tools the model may see, how tool results return to the model, when a failed tool can be retried, when a child task must be awaited, and when the final answer can close the turn.
...
Provider APIs alone cannot run an assistant. Pioneer needs a layer that can call a provider repeatedly, execute tools between calls, update UI state while work runs, and preserve enough state to recover from interruptions.
...
Chat mode handles direct conversations where tools are unnecessary or risky. Agent mode lets Pioneer inspect files, run tools, call MCP servers, use skills, and coordinate subagents. The two modes use different execution paths and provider settings.
...
sequenceDiagram
  ...
  Gateway->>Agent: StartTurn(history, input, capabilities, model, provider, permission_profile, security_snapshot)
  ...
  Provider-->>Agent: visibleTools + memory.activeRecall envelope
  Agent->>Hooks: turn.post_preflight_prompt_context
  Agent->>Hooks: turn.pre_prompt_compile
    Agent->>Prompt: compile system prompt bundle + Current Permissions section when needed
  ...
  loop provider/tool rounds
    ...
    Agent->>Tools: evaluate permissions, enforce security snapshot, execute tool calls
...
1. The gateway supplies historical `ChatMessage` values from completed turns. When a retained history message has artifacts, the message can include compact artifact refs beside the original text.
2. The agent builds the current user message from `UserInput` and turn capability summaries.
...
5. The agent resolves active skills from explicit capabilities, path matches, explicit references, and implicit policy, then builds a compact skills prompt.
...
11. The agent compiles the prompt bundle through `pioneer-promt`. Restricted or narrowed permission profiles add a `Current Permissions` runtime section.
...
The permission profile and execution security snapshot are also carried through this path. They are not prompt-only hints: the tools runtime receives the profile in `PermissionEvaluationContext`, evaluates `allow`, `ask`, or `deny`, and enforces the snapshot's filesystem, network, process, and sandbox policy before side effects. The prompt section exists so the model can plan around user approval boundaries, not so the model becomes the enforcement mechanism.
...
- memory deterministic recall summary and, when available, the active recall planning input owned by the memory subsystem. The active recall payload can include durable-memory planning and thread-episodic planning.
...
The gateway listener persists these events through `CrudStore` before publishing committed notifications to clients. UI state and database state then reflect the same committed sequence.
...
That is the reason tool permission evaluation and output policies live below the agent loop. The agent needs a model-visible result, but the tools runtime must decide whether the action may execute, and the gateway timeline, storage layer, and recovery layer may need different projections of the same output. See [Tools System](/architecture/tools) and [Permission System](/architecture/permissions).
...
- [Provider System](/architecture/providers) explains how API-provider `ChatRequest` values reach OpenAI, Anthropic, Gemini, Ollama, Bedrock, and compatible endpoints.
- [CLI Runtime Architecture](/architecture/cli-runtime) explains the separate process/session path for Codex CLI-backed agent runtime turns.
- [Tasks And Subagents](/architecture/tasks) explains the task tools and child-thread execution path.
- [Permission System](/architecture/permissions) explains turn permission profiles, execution security snapshots, sandbox/resource policy, approval prompts, audit events, and task permission caps.
