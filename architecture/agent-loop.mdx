...
Domain side work is attached through the hook runtime. The loop dispatches lifecycle phases such as `turn.pre_policy`, `turn.pre_prompt_context`, `turn.pre_tool_materialization`, `turn.pre_prompt_compile`, and `turn.post_turn`; hooks contribute typed policy, context, prompt sections, tool bundles, and diagnostics. The loop should not contain domain-specific branches for memory extraction, recall policy, or similar future subsystems.
...
4. The agent dispatches pre-prompt-context hooks, such as deterministic and active memory recall.
5. The agent materializes task tools if task orchestration is available.
6. The agent dispatches pre-tool-materialization hooks, such as memory tool-bundle contribution.
7. The agent dispatches pre-prompt-compile hooks, such as memory prompt contract rendering.
8. The agent compiles the prompt bundle through `pioneer-promt`.
9. The agent materializes MCP tools and dynamic skill tools.
10. The tool router exposes only model-visible tool definitions for the current round.
11. Recovered tool context is appended as assistant tool-call and tool-result messages when a turn is being recovered.
...
| `turn.pre_prompt_context` | Runs deterministic recall and active recall. |
| `turn.pre_tool_materialization` | Contributes `memory_search`, `memory_get`, `memory_remember`, and `memory_forget` when policy allows. |
