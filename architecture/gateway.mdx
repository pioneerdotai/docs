...
- turn lifecycle, permission profile materialization, approval requests, and agent runtime wiring
...
<Steps>
  ...
  <Step title="Persist initial state">
    The gateway materializes the effective `TurnPermissionProfileSnapshot` and calls the turn-start persistence path that writes the thread, sandbox policy, turn, turn input, status history, and profile-selected permission audit event.
  </Step>
  ...
  <Step title="Dispatch execution">
    `AgentManager::start_turn` receives thread mode, provider/model or CLI runtime backend, input, history, skill policy, and the permission profile. From this point the agent runtime owns the turn execution.
  </Step>
...
- [Permission System](/architecture/permissions) explains turn permission profiles, approval brokers, and audit events.
