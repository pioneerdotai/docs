...
Pioneer is organized around one rule: the gateway is the product core. Clients connect to it, but the gateway owns the durable state, runtime state, model access, tool execution, task execution, MCP runtime, skills runtime, settings, and history.
...
If you are new to the codebase, start here, then read [Gateway](/architecture/gateway), [Agent Loop](/architecture/agent-loop), and [Prompt And Context](/architecture/prompt). Those three pages explain the central path from a client message to an LLM request and back to persisted timeline events.
...
<AccordionGroup>
  <Accordion title="Starting a turn">
    A client calls `turn/start` with a `thread_id`, `turn_id`, selected mode, model, provider, sandbox mode, and user input. The gateway updates the thread manager, persists the turn start through `CrudStore::materialize_turn_start`, ensures an agent runtime exists for the thread, loads conversation history, loads workspace skill policy, and dispatches the turn into `AgentManager::start_turn`.
  </Accordion>

  <Accordion title="Building model input">
    The gateway builds historical chat messages from completed turns. If history is close to the configured context budget, it compresses older turns into a summary with a provider call. The agent then compiles the system prompt with `pioneer-promt`, resolves skills, materializes MCP and task tools, builds the current user message, appends retained tool context for recovery when needed, and sends a `ChatRequest` through the selected provider.
  </Accordion>
  ...
  <Accordion title="Loading MCP and skills">
    MCP servers and skills are installed per gateway and per workspace policy. At turn time, enabled and allowed items are resolved into prompt context and dynamic tools. Disabled or non-implicit items stay installed but are not automatically exposed to the model.
  </Accordion>
</AccordionGroup>
...
Pioneer is early-stage software. Some protocol and storage shapes are ahead of the desktop UI. For example, the protocol and database model support workspaces, but the current desktop flow creates one default workspace automatically and does not yet expose full workspace creation and switching.
...
- [Prompt And Context](/architecture/prompt) explains how history, prompt sections, skills, retries, and runtime facts become model input.
