...
<AccordionGroup>
  ...
  <Accordion title="Building model input">
    The gateway builds historical chat messages from completed turns. If history is close to the configured context budget, it compresses older turns into a summary with a provider call. The agent runs turn hooks for policy, prompt context, tool materialization, and prompt sections; compiles the system prompt with `pioneer-promt`; resolves skills; materializes MCP and task tools; builds the current user message; appends retained tool context for recovery when needed; and sends a `ChatRequest` through the selected provider.
  </Accordion>
  ...
  <Accordion title="Recalling and writing memory">
    Memory hooks classify the turn, recall relevant durable facts, expose memory tools when policy allows, render the memory prompt contract, and run post-turn extraction after successful turns. The memory service owns scopes, ranking, canonical keys, dedupe, candidate scoring, tombstones, and memvid-backed storage.
  </Accordion>
