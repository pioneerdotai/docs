...
Pioneer now has turn permission profiles, execution security snapshots, tool approval requests, sandbox/resource policy, backend enforcement status, and durable permission audit events. This gives tools, MCP, skills, computer use, shell execution, web access, and task/subagent launches a shared gateway-owned security model. Use separate gateways when the work should live on a separate host, account, filesystem, credential set, or network.
