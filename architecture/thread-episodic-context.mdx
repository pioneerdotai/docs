...
Thread episodic context is Pioneer's searchable conversation history. It solves a different problem from durable agent memory.
...
This split keeps memory focused. A thread can contain thousands of temporary thoughts, failed attempts, and one-off details. Most of that should remain searchable history rather than permanent agent memory.
...
Both must stay bounded. Workspace-wide recall can return a lot of context, so the service applies query plans, filters, candidate limits, ranking, and prompt budgets before anything reaches the model.
