...
| `crates/protocol` | library | Public JSON-RPC contract, request/response types, notification types, domain DTOs, turn permission profile/approval/audit types, method constants, and schema export. |
| `crates/client` | library | Shared shell-neutral client core: gateway transport, request helpers, notification reducers, read models, selectors, timeline rows, composer workflows, permission-mode display, provider/model helpers, skills/MCP/settings/artifact presentation, and schema contracts. |
...
| `crates/tools` | library | Built-in tool registry, permission evaluation and approval brokering, shell sessions, filesystem tools, patch application, grep, web search/fetch/download, computer use, dynamic tool extension bundles, output policies, retry classification. |
