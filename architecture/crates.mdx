...
Pioneer is a Cargo workspace with twenty Rust crates. The split is intentionally direct: public protocol, gateway orchestration, agent execution, hook runtime, provider adapters, tools, artifacts, tasks, MCP, skills, prompt compilation, agent memory, secret storage, and durable storage are separate layers.
...
| How does agent memory recall and write durable facts? | `crates/agent/src/memory.rs`, `crates/gateway/src/memory_runtime.rs`, then `crates/memory/src/lib.rs` |
