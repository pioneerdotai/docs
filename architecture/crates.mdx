...
Pioneer is a Cargo workspace with sixteen Rust crates. The split is intentionally direct: public protocol, gateway orchestration, agent execution, provider adapters, tools, tasks, MCP, skills, prompt compilation, and storage are separate layers.
...
That shape keeps changes smaller. Adding a provider should mostly touch `provider` and gateway provider listing/key handling. Changing prompt section ordering should mostly touch `promt` and prompt tests. Adding a task event should touch `protocol`, `tasks`, `crud`, and gateway notifications, but not desktop-specific execution code.
...
| `crates/cli` | binary | Command-line utility and service management entry point. |
