...
Pioneer is a Cargo workspace with nineteen Rust crates. The split is intentionally direct: public protocol, gateway orchestration, agent execution, hook runtime, provider adapters, tools, tasks, MCP, skills, prompt compilation, agent memory, secret storage, and durable storage are separate layers.
