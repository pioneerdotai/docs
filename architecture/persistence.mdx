...
Persistence is not just storage for finished conversations. It is part of the runtime contract. The gateway uses it to make active turns visible, recover from interrupted work, audit which prompt, skills, MCP bindings, permission profile, and execution security snapshot were used, track MCP catalogs, reconcile tasks, and keep clients in sync after reconnect.
