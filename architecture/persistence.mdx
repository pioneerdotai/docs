...
Persistence is not just storage for finished conversations. It is part of the runtime contract. The gateway uses it to make active turns visible, recover from interrupted work, audit which prompt, skills, MCP bindings, and permission profile were used, track MCP catalogs, reconcile tasks, and keep clients in sync after reconnect.
...
| Turns | turn rows, turn input, turn items, turn item attempts, status history, timeline events, permission profile snapshots, permission audit events |
