...
description: "SQLite, SeaORM entities, CRUD repositories, projectors, events, and runtime recovery tables."
---

Pioneer persists gateway state in SQLite through SeaORM. The persistence layer is split into entity definitions, migrations, SQLite helpers, and the `CrudStore` repository/projector facade used by the gateway and task runtime.
...
Do not store raw secrets in ordinary domain tables. Provider keys and MCP secrets belong in gateway settings/secret storage, while database records should contain redacted config or secret refs.
