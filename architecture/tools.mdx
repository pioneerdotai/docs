...
Dynamic extension tools that are not in the built-in domain map are independent of preflight domain selection and `request_tools`. If an MCP server or skill contributes a turn-specific tool, the final visibility pass keeps it available as a dynamic extension when it is registered and has a handler.
