...
Tool execution is protected by the turn permission system. Each turn carries an effective permission profile, and the tools runtime evaluates side-effecting actions before execution. Depending on policy, a tool action is allowed, blocked, or converted into a client approval request.
...
<Warning>
  Permission profiles are an application-level policy boundary, not a separate OS sandbox. A remote gateway means the tool runs on the remote host. A local gateway means it runs as the local OS user. Use gateway separation or host-level isolation when the machine boundary matters.
</Warning>
...
Every tool invocation receives a `PermissionEvaluationContext` containing the workspace, thread, turn, and `TurnPermissionProfileSnapshot`. Before execution, the orchestrator extracts a `PermissionIntent`, classifies the action kind, and evaluates it against the profile's `ToolPermissionPolicySnapshot`.
