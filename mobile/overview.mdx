...
| Composer | Attach files, attach skills and MCP capabilities, choose a permission mode, and send text turns. |
...
<Warning>
  Tool calls, local paths, MCP servers, provider requests, and task execution happen on the gateway host, not on the phone. Permission modes control when Pioneer asks before tool actions, but they do not move execution onto the mobile device.
</Warning>
